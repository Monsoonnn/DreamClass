...
public class BookVR : MonoBehaviour {
    ...
    public float pokeDepthThreshold = 0.02f; // 2cm vào trong book
    public float pinchThresholdToGrab = 0.7f; // Pinch strength để grab
    ...
    void Start() {
        ...
        // Initialize book plane for poke detection
    ...
    void InitializeHandTracking() {
        ...
        if (rightHandSkeleton != null)
            StartCoroutine(GetFingerTipWhenReady(rightHandSkeleton, OVRSkeleton.BoneId.Hand_IndexTip, true));
        if (leftHandSkeleton != null)
            StartCoroutine(GetFingerTipWhenReady(leftHandSkeleton, OVRSkeleton.BoneId.Hand_IndexTip, false));

        Debug.Log(rightHand != null || leftHand != null ? "✓ Hand tracking initialized" : "⚠️ Hand tracking not found");
    }

    IEnumerator GetFingerTipWhenReady( OVRSkeleton skeleton, OVRSkeleton.BoneId boneId, bool isRightHand ) {
        while (skeleton.Bones.Count == 0) yield return null;

        foreach (var bone in skeleton.Bones) {
            if (bone.Id == boneId) {
                if (isRightHand) rightIndexTip = bone.Transform;
                else leftIndexTip = bone.Transform;
                Debug.Log($"✓ {(isRightHand ? "Right" : "Left")} index finger found");
                break;
            }
        }
    }
    ...
    void Update() {
        ...
        // Handle input based on tracking mode
    ...
    void HandleHandTrackingInput() {
        ...
        Transform fingerTip = (activeHand == rightHand) ? rightIndexTip : leftIndexTip;
        ...
        // Method 2: Poke detection (finger penetrates book plane)
        if (enablePokeInteraction) {
            HandlePokeInteraction(fingerTip, activeHand, isPinching);
        }

        // Release on unpinch
        if (!isPinching && pageDragging && isPinchGrabbing) {
            OnHandRelease();
        }
    }
    ...
    void UpdateDebugRays() {
        ...
        if (handDebugLineRenderer != null && isHandTrackingActive) {
            Transform fingerTip = rightIndexTip ?? leftIndexTip;
            if (fingerTip != null) {
